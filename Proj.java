import java.awt.EventQueue;
	...
	class DC_voltage_source {
		//класс, описывающий источник Ёƒ—
		double EMF;//Ёƒ— элемента
		boolean on;//работает/сломан
		int eff;//работоспособность элемента
		int enter;//вход/выход/просто коммутаци€
		DC_voltage_source neighbors[];//массив, содержащий соседей элемента
		JTextField label;//графическое отображение элемента
		
		//
		DC_voltage_source control_transfer(DC_voltage_source obj) {
			int a=obj.neighbors.length;
			int random_neighbor=(int)+(Math.random()*a);
			return(obj.neighbors[random_neighbor]);
		}
		//
		DC_voltage_source enter_finder(DC_voltage_source[][] obj) {
		...
		public DC_voltage_source (double EMF, boolean on, int eff, int enter){
			this.EMF=EMF;
			this.on=on;
			this.eff=eff;
			this.enter=enter;}
	...
	public class Proj {
		...
		private class SwingAction extends AbstractAction {
			...
			public void actionPerformed(ActionEvent e) {
						...
						for(int i = 0; i < 7; i++){
							for(int j=0; j<7; j++){
								...
								case 5:
									...
									break;
								case 6:
									agent[i][j]=new DC_voltage_source(0.5,true,0,0);
									break;
								}
							}
						}
						...
						agent[1][0].neighbors=new DC_voltage_source[]{agent[0][0],agent[1][1],agent[2][0]};
						...
						agent[1][6].neighbors=new DC_voltage_source[]{agent[0][6],agent[1][5],agent[2][6]};
						...
						agent[2][6].neighbors=new DC_voltage_source[]{agent[1][6],agent[2][5],agent[3][6]};
						agent[3][0].neighbors=new DC_voltage_source[]{agent[2][0],agent[3][1],agent[4][0]};
						...
						agent[3][6].neighbors=new DC_voltage_source[]{agent[2][6],agent[3][5],agent[4][6]};
						agent[4][0].neighbors=new DC_voltage_source[]{agent[3][0],agent[4][1],agent[5][0]};
						...
						agent[4][6].neighbors=new DC_voltage_source[]{agent[3][6],agent[4][5],agent[6][6]};
						agent[5][0].neighbors=new DC_voltage_source[]{agent[4][0],agent[5][1],agent[6][0]};
						...
						agent[5][6].neighbors=new DC_voltage_source[]{agent[4][6],agent[5][5],agent[6][6]};
					    ...
					    obj=agent[0][0].enter_finder(agent);
					    path=obj.finder(obj.enter_finder(agent), U_zad);
					    int i=0;
							do {
								//System.out.print(path[i].EMF+" ");
								path[i].label.setBackground(Color.PINK);
								i++;}
							while (path[i]!=null);
					}
				}
